  ...
  return <div className="section-page">
    <div className="section-heading">
      <div>
        ...
        <h2>Admin Logs</h2>
        <p>Review authentication, account, permission and settings activity.</p>
      </div>
    ...
    <section className="settings-card admin-logs-card admin-logs-page-card">
      <div className="settings-card-heading admin-logs-heading">
        ...
        <div>
          ...
          <h3>Audit history</h3>
          <p>Records are maintained locally in this browser and are available only to administrator accounts.</p>
        </div>
        <div className="admin-log-count"><strong>{adminLogs.length}</strong><span>records</span></div>
      </div>
        ...
        {filteredLogs.length === 0 ? (
          ...
          <div className="admin-log-list">
            {filteredLogs.map((log) => {
              ...
              const positive = ["LOGIN", "CREATE_ACCOUNT", "ROLE_CHANGED", "SETTING_CHANGED"].includes(log.action);
      ...
      <div className="admin-log-footer"><ShieldCheck size={14}/> Admin-only audit trail · {filteredLogs.length} of {adminLogs.length} records shown.</div>
    </section>
  </div>;
}
