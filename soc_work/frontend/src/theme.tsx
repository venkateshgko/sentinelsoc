...
// Every new admin login starts in dark mode. Once the admin changes the
// theme, that choice is preserved while the current session is active and
// therefore applies consistently to every protected page.
...
export function ThemeProvider({ children }: { children: ReactNode }) {
  ...
  // - new authenticated session => dark unless the admin has already changed
  //   the theme during THIS browser session
  useEffect(() => {
    if (!user) {
      try {
  ...
  // Keep the DOM in sync and preserve the admin's selection for the current
